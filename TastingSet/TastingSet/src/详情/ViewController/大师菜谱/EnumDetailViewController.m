...
#define myweburl @"http://app.legendzest.cn/index.php?g=api241&m=cookbook&a=getinfo"
#define getenumUrl @"http://app.legendzest.cn/index.php?g=api241&m=cookbook&a=info"

@interface EnumDetailViewController ()<ToolViewDelegate, WKNavigationDelegate, UIScrollViewDelegate>

@property (nonatomic, strong) RoomModel *headModel;
@property (nonatomic, strong) EnumHeadView *headView;
...
@implementation EnumDetailViewController
...
- (void)loadData {
    [CustomActivityView startAnimating];
    [[NetDataEngine sharedInstance] requesPostDataFrom:getenumUrl parameters:[self getParameters] success:^(id responsData) {
        [CustomActivityView stopAnimating];
...
- (void)addHeadView {
    if (self.headView == nil) {
        self.headView = [[EnumHeadView alloc] initWithFrame:CGRectMake(0, -400, KScreenWidth, 400)];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if ([url hasPrefix:@"viewcookbook"]) {
        ...
        detailVC.cid = self.cid;
        detailVC.myId = [url substringFromIndex:15];
